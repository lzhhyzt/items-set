<template>
  <div class="addAdmin">
    <div class="addAdmin__body">
      <div v-if="getFBaseInfoAdd == '0'">
      <el-form ref="form" :model="form" label-width="80px" :rules="rules">
        <el-form-item label="账户" prop="username">
          <el-input v-model="form.username" size="small" placeholder="请输入账户" clearable></el-input>
        </el-form-item>
...
export default {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$confirm('是否添加该用户?', '提示', {
            ...
            $http.post(apis.addFUserDetail, this.form)
            .then(res => {
              if (res.code== 1) {
                this.$message({
                  type: 'success',
  ...
  watch: {
    getFBaseInfoAdd: function(newVal,oldVal) {
      if (newVal != '0') {
